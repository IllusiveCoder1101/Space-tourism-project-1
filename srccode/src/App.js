 ...
 const App = () => {
  ...
  {<aside className={(visibility)?"sidebar show-sidebar":"sidebar close-sidebar "}>
      ...
      <ul className="navbar1">
          <li>
          <button type="button" disabled={(move)?true:false}  className={(shift)?(shift==='home')?"btn00 active1":"btn00":(ctr===0)?"btn00 active1":"btn00"} onClick={()=>{setCtr(0);shifting('home')}}>
            HOME
          </button>
      </li>
      <li>
          <button type="button" disabled={(move)?true:false} className={(shift)?(shift==='destination')?"btn01 active1":"btn01":(ctr===1)?"btn01 active1":"btn01"} onClick={()=>{setCtr(1);shifting('destination')}}>
            DESTINATION
          </button>
      </li>
      <li>
          <button type="button" disabled={(move)?true:false} className={(shift)?(shift==='crew')?"btn02 active1":"btn02":(ctr===2)?"btn02 active1":"btn02"} onClick={()=>{setCtr(2);shifting('crew')}}>
            CREW
          </button>
      </li>
      <li>
          <button type="button" disabled={(move)?true:false} className={(shift)?(shift==='technology')?"btn03 active1":"btn03":(ctr===3)?"btn03 active1":"btn03"} onClick={()=>{setCtr(3);shifting('technology')}}>
          TECHNOLOGY
          </button>
      </li>
  </ul>
...
const page=(ctr,nxt="")=>{
    ...
    if (ctr===1 || nxt==="destination" ){
      return <Destination/>
    }
    if (ctr===2 || nxt==="crew"){
      return <Crew/>
    }
    if (ctr===3 || nxt==="technology"){
      return <Technology/>
    }
}
